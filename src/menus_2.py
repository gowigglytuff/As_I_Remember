...
class GameActionDialogue(object):
    ...
    def __init__(self, gc_input, gd_input):
        ...
        self.gd_input = gd_input
        self.screen = self.gc_input.screen
        gd_input.add_overlay(self.OVERLAYNAME, Overlay(gc_input, gd_input, self.OVERLAYNAME, 780, 680, Spritesheet("assets/menu_images/testing_menu.png", 200, 100)))
...
class StatsMenu(object):
    ...
    def __init__(self, gc_input, gd_input):
        ...
        self.gd_input = gd_input
        self.screen = self.gc_input.screen
        gd_input.add_overlay(self.OVERLAYNAME, Overlay(gc_input, gd_input, self.OVERLAYNAME, 880, 20, Spritesheet("assets/menu_images/use_menu.png", 100, 100)))
        self.overlay = self.OVERLAYNAME
        self.offset_x = 10
        self.offset_y = 20
        self.x = self.gd_input.overlay_list[self.overlay].x + self.offset_x
...
class ConversationOptionsMenu(MenuTemporary):
    ...
    def __init__(self, gc_input, gd_input, menu_item_list):
        super().__init__(gc_input, gd_input, menu_item_list)
        gd_input.add_overlay(self.OVERLAYNAME, TextBox(gc_input, gd_input, self.OVERLAYNAME, 250, 550, Spritesheet("assets/menu_images/text_box.png", 500, 150)))
        self.overlay = self.OVERLAYNAME
...
class SellingMenu(MenuTemporary):
    ...
    def choose_option(self):
        self.gd_input.menu_list[YesNoMenu].set_menu()
