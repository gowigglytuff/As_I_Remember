...
class Ringside(Room):
    ...
    def add_room_doors(self):
        self.gd_input.room_list[self.name].add_room_door("ringside_to_computer_centre", Door("Ringside", "computer_centre", 11, 84, 3, 3, "ringside_to_computer_centre", [Direction.UP]))
        self.gd_input.room_list[self.name].add_room_door("ringside_to_hornby_creative", Door("Ringside", "hornby_creative", 12, 76, 4, 3, "ringside_to_hornby_creative", [Direction.UP]))
        self.gd_input.room_list[self.name].add_room_door("ringside_to_to_the_moon", Door("Ringside", "to_the_moon", 41, 81, 2, 3, "ringside_to_to_the_moon", [Direction.UP]))
        self.gd_input.room_list[self.name].add_room_door("ringside_to_hornby_realestate", Door("Ringside", "hornby_realestate", 39, 75, 1, 3, "ringside_to_hornby_realestate", [Direction.UP]))
        self.gd_input.room_list[self.name].add_room_door("ringside_to_island_potters_1", Door("Ringside", "island_potters", 39, 63, 2, 3,  "ringside_to_island_potters_1", [Direction.UP]))
        self.gd_input.room_list[self.name].add_room_door("ringside_to_island_potters_2", Door("Ringside", "island_potters", 41, 63, 4, 3, "ringside_to_island_potters_2", [Direction.UP]))
        self.gd_input.room_list[self.name].add_room_door("ringside_to_bike_shop", Door("Ringside", "bike_shop", 40, 91, 2, 6, "ringside_to_bike_shop", [Direction.UP]))

    def add_room_characters(self):
        self.gd_input.add_character("Deb", GenericNPC(30, 76, self.gc_input, self.gd_input, Spritesheet("assets/NPC_sprites/Deb_CS.png", 32, 40), "Deb", self.name, "Something strange is going on around here, have you heard about the children disapearing? Their parents couldn't even remember their names...", "stay_left", Direction.LEFT, Spritesheet("assets/NPC_sprites/faces/DebFace.png", 150, 150)))
        ...
        self.gd_input.add_character("Clair", GenericNPC(30, 60, self.gc_input, self.gd_input, Spritesheet("assets/NPC_sprites/Clair_CS.png", 32, 40), "Clair", self.name, "I love this place but sometimes I feel like I should just pack everything up and run far away...", "square", Direction.DOWN, Spritesheet("assets/NPC_sprites/faces/ClairFace.png", 150, 150)))
        self.gd_input.add_character("Clayton", GenericNPC(23, 72, self.gc_input, self.gd_input, Spritesheet("assets/NPC_sprites/Clayton_CS.png", 32, 40), "Clayton", self.name, "Nyah! Why is everybody on this island so weird?? And have you seen the cost of icecream??? Of all the days to forget my boomerang at home...", "spin", Direction.DOWN, Spritesheet("assets/NPC_sprites/faces/ClaytonFace.png", 150, 150)))
        self.gd_input.add_character("Marilyn", GenericNPC(29, 76, self.gc_input, self.gd_input, Spritesheet("assets/NPC_sprites/Marilyn_CS.png", 32, 40), "Marilyn", self.name, "Have you seen my son around? He's always running off and getting up to mischief", "stay_right", Direction.RIGHT, Spritesheet("assets/NPC_sprites/faces/MarilynFace.png", 150, 150)))
        self.gd_input.add_character("Maggie", GenericNPC(21, 82, self.gc_input, self.gd_input, Spritesheet("assets/NPC_sprites/Maggie.png", 32, 40), "Maggie", self.name, "This outfit makes me feel really cool and powerful, so I've decided I'm going to wear it everywhere.", "stand_still", Direction.DOWN, Spritesheet("assets/NPC_sprites/faces/MaggieFace.png", 150, 150)))
        self.gd_input.add_character("Laurie", GenericNPC(32, 101, self.gc_input, self.gd_input, Spritesheet("assets/NPC_sprites/Laurie.png", 32, 40), "Laurie", self.name, "Have you seen my drink anywhere?", "square", Direction.DOWN, Spritesheet("assets/NPC_sprites/faces/LaurieFace.png", 150, 150)))
        self.gd_input.add_character("Ian", GenericNPC(23, 59, self.gc_input, self.gd_input, Spritesheet("assets/NPC_sprites/Ian_CS.png", 32, 40), "Ian", self.name, "Shoot, I left my wallet in the farm truck, guess I'll be paying in charm again...", "stay_front", Direction.DOWN, Spritesheet("assets/NPC_sprites/faces/IanFace.png", 150, 150)))
    ...
    def add_room_props(self):
            ...
            self.gd_input.add_prop(AppleTree.assign_dict_key(), AppleTree(38, 69, self.gc_input, self.gd_input, self.name))
            self.gd_input.add_prop(BenchHorizontal.assign_dict_key(), BenchHorizontal(28, 78, self.gc_input, self.gd_input, self.name))
            self.gd_input.add_prop(BenchHorizontal.assign_dict_key(), BenchHorizontal(29, 70, self.gc_input, self.gd_input, self.name))
            self.gd_input.add_prop(BenchVertical.assign_dict_key(), BenchVertical(26, 73, self.gc_input, self.gd_input, self.name))
            self.gd_input.add_prop(BenchVertical.assign_dict_key(), BenchVertical(33, 73, self.gc_input, self.gd_input, self.name))
            self.gd_input.add_prop(BenchVertical.assign_dict_key(), BenchVertical(22, 69, self.gc_input, self.gd_input, self.name))
            self.gd_input.add_prop(BenchVertical.assign_dict_key(), BenchVertical(21, 79, self.gc_input, self.gd_input, self.name))

            self.gd_input.add_prop(BenchVertical.assign_dict_key(), BenchVertical(26, 84, self.gc_input, self.gd_input, self.name))
            self.gd_input.add_prop(BenchVertical.assign_dict_key(), BenchVertical(32, 84, self.gc_input, self.gd_input, self.name))
            self.gd_input.add_prop(BenchVertical.assign_dict_key(), BenchVertical(26, 87, self.gc_input, self.gd_input, self.name))
            self.gd_input.add_prop(BenchVertical.assign_dict_key(), BenchVertical(32, 87, self.gc_input, self.gd_input, self.name))
            self.gd_input.add_prop(BenchHorizontal.assign_dict_key(), BenchHorizontal(26, 83, self.gc_input, self.gd_input, self.name))
            self.gd_input.add_prop(BenchHorizontal.assign_dict_key(), BenchHorizontal(29, 83, self.gc_input, self.gd_input, self.name))
            self.gd_input.add_prop(BenchHorizontal.assign_dict_key(), BenchHorizontal(27, 89, self.gc_input, self.gd_input, self.name))
            self.gd_input.add_prop(LargeStone.assign_dict_key(), LargeStone(31, 89, self.gc_input, self.gd_input, self.name))
            self.gd_input.add_prop(LargeStone.assign_dict_key(), LargeStone(30, 89, self.gc_input, self.gd_input, self.name))
            self.gd_input.add_prop(LargeStone.assign_dict_key(), LargeStone(41, 83, self.gc_input, self.gd_input, self.name))
            self.gd_input.add_prop(Planter.assign_dict_key(), Planter(32,77, self.gc_input, self.gd_input, self.name))

            #self.gd_input.add_prop(Tree.assign_dict_key(), Tree(37, 69, self.gc_input, self.gd_input, self.name))
            self.gd_input.add_prop(PlumTree.assign_dict_key(), PlumTree(29, 74, self.gc_input, self.gd_input, self.name))
            self.gd_input.add_prop(BenchHorizontal.assign_dict_key(), BenchHorizontal(37, 70, self.gc_input, self.gd_input, self.name))

            self.gd_input.add_prop(PicnicTable.assign_dict_key(), PicnicTable(17, 82, self.gc_input, self.gd_input, self.name))

            self.gd_input.add_prop(PicnicTable.assign_dict_key(), PicnicTable(9, 65, self.gc_input, self.gd_input, self.name))
            self.gd_input.add_prop(PicnicTable.assign_dict_key(), PicnicTable(9, 69, self.gc_input, self.gd_input, self.name))
            self.gd_input.add_prop(PicnicTable.assign_dict_key(), PicnicTable(13, 65, self.gc_input, self.gd_input, self.name))
            self.gd_input.add_prop(PicnicTable.assign_dict_key(), PicnicTable(13, 69, self.gc_input, self.gd_input, self.name))
...
class ComputerCentreRoom(Room):
    ...
    def add_room_doors(self):
        self.gd_input.room_list[self.name].add_room_door("computer_centre_to_ringside", Door("computer_centre", "Ringside",  3, 4, 11, 85, "computer_centre_to_ringside", [Direction.DOWN]))
...
class HornbyCreativeRoom(Room):
    ...
    def add_room_doors(self):
        self.gd_input.room_list[self.name].add_room_door("hornby_creative_to_ringside", Door("hornby_creative", "Ringside", 4, 4, 12, 77, "hornby_creative_to_ringside", [Direction.DOWN]))
...
class ToTheMoon(Room):
    ...
    def add_room_doors(self):
        self.gd_input.room_list[self.name].add_room_door("to_the_moon_to_ringside", Door("to_the_moon", "Ringside", 2, 4, 41, 82, "to_the_moon_to_ringside", [Direction.DOWN]))
...
class HornbyRealestate(Room):
    ...
    def add_room_doors(self):
        self.gd_input.room_list[self.name].add_room_door("hornby_realestate_to_ringside", Door("hornby_realestate", "Ringside", 1, 4, 39, 76, "hornby_realestate_to_ringside", [Direction.DOWN]))
...
class IslandPotters(Room):
    ...
    def add_room_doors(self):
        self.gd_input.room_list[self.name].add_room_door("island_potters_to_ringside_1", Door("island_potters", "Ringside", 2, 4, 39, 64, "island_potters_to_ringside_1", [Direction.DOWN]))
        self.gd_input.room_list[self.name].add_room_door("island_potters_to_ringside_2", Door("island_potters", "Ringside", 4, 4, 41, 64, "island_potters_to_ringside_2", [Direction.DOWN]))
...
class BikeShop(Room):
    ...
    def add_room_doors(self):
        self.gd_input.room_list[self.name].add_room_door("bike_shop_to_ringside", Door("bike_sho", "Ringside", 2, 7, 40, 92, "bike_shop_to_ringside", [Direction.DOWN]))
